...
**svcctl** is a lightweight multi-app service manager for local development. It runs a background daemon that keeps services alive, with a CLI for control and a Textual-based TUI for monitoring.
...
1. `cli.py` — Click command group defining all CLI commands (`start`, `stop`, `restart`, `status`, `logs`, `ui`, `daemon`). Entry point: `svcctl.cli:main`.
